...
export default function AboutPageEditor() {
  ...
  useEffect(() => {
    const loadAboutData = async () => {
      try {
        setIsLoading(true);
        const response = await fetch("/api/maktabah");
        if (response.ok) {
        ...
        console.error("Error loading maktabah data:", error);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage("Maktabah page saved successfully!");
        setTimeout(() => setMessage(""), 3000);
      } else {
        setMessage("Error saving maktabah page: " + result.message);
      }
    } catch (error) {
      setMessage("Error saving maktabah page");
      console.error("Error saving maktabah data:", error);
    } finally {
      setIsSaving(false);
    }
  };
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Maktabah - Page Title"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
