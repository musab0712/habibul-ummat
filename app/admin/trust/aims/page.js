...
export default function AboutPageEditor() {
  ...
  useEffect(() => {
    const loadAboutData = async () => {
      try {
        setIsLoading(true);
        const response = await fetch("/api/trust-aims");
        if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage("Trust aims page saved successfully!");
        setTimeout(() => setMessage(""), 3000);
      } else {
        setMessage("Error saving trust aims page: " + result.message);
      }
    } catch (error) {
      setMessage("Error saving trust aims page");
      console.error("Error saving trust aims data:", error);
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <div className="px-6 py-4">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-800">
              Aims and Objectives Page Management
            </h1>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Aims and Objectives - Page Title"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
